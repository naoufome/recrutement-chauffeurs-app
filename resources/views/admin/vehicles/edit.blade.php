...
<div class="space-y-6">
                    ...
                    <h2 class="text-2xl font-bold leading-7 text-blue-800 dark:text-blue-300 sm:truncate sm:text-3xl sm:tracking-tight">
                        <div class="flex items-center">
                            ...
                            <svg class="h-8 w-8 mr-3 text-blue-600 dark:text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l-4.655 4.655a2.652 2.652 0 1 1-3.75-3.75l4.654-4.655M11.42 15.17l-1.414-1.414M11.42 15.17 15.17 11.42" />
    ...
    <!-- Formulaire -->
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl rounded-lg border border-blue-100 dark:border-blue-800">
         <form method="POST" action="{{ route('admin.vehicles.update', $vehicle->id) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-4"> {{-- Inner container --}}
                ...
                <div>
                    ...
                    <input type="text" name="plate_number" id="plate_number" required value="{{ old('plate_number', $vehicle->plate_number) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="type" id="type" value="{{ old('type', $vehicle->type) }}" placeholder="Ex: Fourgon, Camionnette, PL..."
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="number" name="year" id="year" value="{{ old('year', $vehicle->year) }}" min="1900" max="{{ date('Y') + 1 }}" step="1" placeholder="{{ date('Y') }}"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                ...
                <div>
                    ...
                    <select name="is_available" id="is_available" required
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                        {{-- Utilise la valeur booléenne/int directement --}}
                        <option value="1" @selected(old('is_available', $vehicle->is_available) == 1)>Oui</option>
                        <option value="0" @selected(old('is_available', $vehicle->is_available) == 0)>Non (ex: en entretien)</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3" placeholder="Entretien à prévoir, particularités..."
                              class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">{{ old('notes', $vehicle->notes) }}</textarea>
