...
<div class="space-y-6">
                    ...
                    <h2 class="text-2xl font-bold leading-7 text-blue-800 dark:text-blue-300 sm:truncate sm:text-3xl sm:tracking-tight">
                        <div class="flex items-center">
                             ...
                             <svg class="h-8 w-8 mr-3 text-blue-600 dark:text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                               ...
                               <path stroke-linecap="round" stroke-linejoin="round" d="M15.375 4.5 19.5 8.625m-1.5-1.5-4.5 4.5m4.5-4.5L15 8.625m1.5-1.5-4.5 4.5m0 0L11.25 7.5m2.625 2.625-4.5 4.5m4.5-4.5L15 12.375" />
                             </svg>
                            Modifier le Critère d'Évaluation
                        </div>
                    </h2>
    ...
    <!-- Formulaire -->
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl rounded-lg border border-blue-100 dark:border-blue-800">
         <form method="POST" action="{{ route('admin.evaluation-criteria.update', $criterion->id) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-4"> {{-- Inner container --}}
                ...
                <div>
                    ...
                    <input type="text" name="category" id="category" list="category_suggestions" value="{{ old('category', $criterion->category) }}" placeholder="Ex: Entretien RH, Technique, Test Conduite..."
                           class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                    <datalist id="category_suggestions">
                         @foreach($categories as $category) {{-- Assumes $categories passed --}}
                ...
                {{-- Description --}}
                 <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">{{ old('description', $criterion->description) }}</textarea>
                ...
                <div>
                    ...
                    <select name="is_active" id="is_active" required
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                        <option value="1" @selected(old('is_active', $criterion->is_active) == 1)>Oui</option>
                        <option value="0" @selected(old('is_active', $criterion->is_active) == 0)>Non</option>
                    </select>
