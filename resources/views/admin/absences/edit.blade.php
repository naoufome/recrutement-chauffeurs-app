...
<div class="space-y-6">
                ...
                <div class="min-w-0 flex-1">
                    <h2 class="text-2xl font-bold leading-7 text-blue-800 dark:text-blue-300 sm:truncate sm:text-3xl sm:tracking-tight">
                        <div class="flex items-center">
                            ...
                            Modifier Absence #{{ $absence->id }}
                        </div>
                    </h2>
                     <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                       Pour {{ $absence->employee->user->name ?? 'N/A' }} - Date: {{ $absence->absence_date->format('d/m/Y') }}
                    </p>
                </div>
    ...
    <!-- Formulaire -->
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl rounded-lg border border-blue-100 dark:border-blue-800">
         <form method="POST" action="{{ route('admin.absences.update', $absence->id) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-4"> {{-- Inner container --}}
                ...
                <div>
                    ...
                    <select name="employee_id" id="employee_id" required
                        ...
                        @foreach($employees ?? [] as $emp) {{-- Assumes $employees passed --}}
                            <option value="{{ $emp->id }}" @selected(old('employee_id', $absence->employee_id) == $emp->id)>
                ...
                <div>
                    ...
                    <input type="date" name="absence_date" id="absence_date" required
                           value="{{ old('absence_date', $absence->absence_date->format('Y-m-d')) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="time" name="start_time" id="start_time"
                               value="{{ old('start_time', $absence->start_time ? Carbon\Carbon::parse($absence->start_time)->format('H:i') : '') }}"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="time" name="end_time" id="end_time"
                               value="{{ old('end_time', $absence->end_time ? Carbon\Carbon::parse($absence->end_time)->format('H:i') : '') }}"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                ...
                <div>
                    ...
                    <input list="reason_type_list" name="reason_type" id="reason_type" value="{{ old('reason_type', $absence->reason_type) }}" placeholder="Ex: Maladie, Congé payé, Injustifiée..."
                           class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                ...
                <div>
                    ...
                    <select name="is_justified" id="is_justified" required
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">
                        {{-- Utilise la valeur booléenne/int directement pour la comparaison --}}
                        <option value="0" @selected(old('is_justified', $absence->is_justified) == 0)>Non</option>
                        <option value="1" @selected(old('is_justified', $absence->is_justified) == 1)>Oui</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-gray-300 sm:text-sm">{{ old('notes', $absence->notes) }}</textarea>
